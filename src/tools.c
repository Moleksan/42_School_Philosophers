...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/19 16:16:51 by moleksan          #+#    #+#             */
/*   Updated: 2024/12/21 11:57:39 by moleksan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	output_status(t_philosopher *p, int i)
{
	...
	if (i == 1)
		printf("%s%lu %d has taken left fork%s\n", PURPLE,
		...
		printf("%s%lu %d has taken right fork%s\n", PINK,
			current_time() - p->start_time, p->philosopher_id, RESET);
	pthread_mutex_unlock(p->simulation_params->print_lock);
}
...
void	check_threads(t_philosopher *p)
{
	...
	if (p->simulation_params->eaten_count
			== p->simulation_params->philosophers_count)
			p->simulation_params->is_over = 1;
	}
	return ;
}
